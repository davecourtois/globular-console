...
/**
 * Globular application panel, display all the application installed on the server.
 */
export class ApplicationsPanel extends HTMLElement {
    ...
    init(callback) {
        ...
        ApplicationController.getAllApplicationInfos((infos) => {
            ...
            for (var i = 0; i < infos.length; i++) {
                ...
                <div id="${application.getId()}_div" class="application-div">
                    ...
                    <paper-tooltip for="${application.getId()}_div" style="font-size: 1rem; min-width: 100px;" role="tooltip" tabindex="-1">${application.getDescription()}</paper-tooltip>
                </div>
                ...
                if (div_ != null) {
                    ...
                    Backend.globular.eventHub.subscribe(`update_application_${application.getId()}_settings_evt`,
                        (uuid) => {
...
export class ApplicationManager extends HTMLElement {
    ...
    constructor() {
        ...
        let displayApplications = () => {
            ...
            ApplicationController.getAllApplicationInfos(
                (applications) => {
        ...
        // Subscribe to the refresh application event.
        Backend.globular.eventHub.subscribe("refresh_application_evt", uuid => { }, evt => {
            displayApplications()
        }, true)
    }
...
export class ApplicationPanel extends HTMLElement {
        ...
        <div id="container">
            <div class="header">
                ...
                <div style="display: flex; width: 32px; height: 32px; justify-content: center; align-items: center; position: relative;">
                    <iron-icon id="hide-btn" icon="unfold-less" style="flex-grow: 1; --iron-icon-fill-color: var(--on-background-color);" icon="add"></iron-icon>
            ...
            <iron-collapse id="collapse-panel">
                ...
                <div class="row">
                    ...
                    <paper-textarea value="${this.application.getDescription()}"></paper-textarea>
                </div>
    ...
    `;
        ...
        let actionsList = new SearchableList("Actions", this.application.getActionsList(),
            (action) => {
                // remove action...
                ApplicationController.removeApplicationAction(application, action, () => {
                    displayMessage("Action " + action + " was removed from application " + application.getId() + "@" + application.getDomain(), 3000)
                }, (err) => {
                    console.log(err)
            ...
            (action) => {
                displayMessage("Action " + action + " was added to application " + application.getId() + "@" + application.getDomain(), 3000)
            },
            (actions) => {

                ApplicationController.getAllActions(
                    (actions) => {
                        ...
                        if (panel == undefined) {
                            ...
                            actions_.forEach(a => {
                                ...
                                actionAddBtn.onclick = () => {
                                    ApplicationController.addApplicationAction(application, a, () => {
                                        displayMessage("Action " + a + " was added to application " + application.getId() + "@" + application.getDomain(), 3000)
    ...
    onDeleteApplication(application) {
        ...
        yesBtn.onclick = () => {
            ...
            ApplicationController.deleteApplication(application, () => {
                displayMessage(
    ...
    onUninstallApplication(application) {
        ...
        yesBtn.onclick = () => {
            ...
            ApplicationController.uninstallApplication(application, () => {
                displayMessage(
                    "<iron-icon icon='communication:message' style='margin-right: 10px;'></iron-icon><div>Application named " +
                    application.getName() +
                    " was uninstall!</div>",
