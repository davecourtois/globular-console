...
import { Backend, generatePeerToken } from "../controllers/Backend";
...
export class Session implements IModel {

    // private members.
    private _id: string;
    private account: Account;
    private state_: SessionState = SessionState.Offline;
    private lastStateTime_: Date = new Date; // Keep track ot the last session state.
    ...
    getSession(callback: (session: resource.Session, domain: string) => void, errorCallback: (err: any) => void) {
        ...
        let _getSession_ = (globule: Globular) => {
            ...
            if (token == null) {
                throw `No token found for domain ${globule.domain}`
            }
    ...
    initData(initCallback: () => void, errorCallback: (err: any) => void) {
        ...
        this.getSession((session: resource.Session, domain: string) => {
            ...
            let globule = Backend.getGlobule(this.account.domain)
            ...
            if(globule.rbacService == null){
                ...
                throw `No RBAC service found for domain ${globule.domain}`
            }
            ...
            AccountController.getAccount(this._id,
                (account: Account) => {
    ...
    fromObject(obj: any) {
        ...
        this._id = obj._id;
        AccountController.getAccount(this._id, (account: Account) => { this.account = account }, (err: any) => { console.log(err) })
    ...
    save(onSave: () => void, onError: (err: any) => void) {
        ...
        if (token_expired == null) {
            throw `No token expired found`
        }

        session.setExpireAt(parseInt(token_expired))
        rqst.setSession(session)

        // TEST if session must be on the user globule or the actual session store.
        let globule = Backend.getGlobule(this.account.domain)
        if (globule == null) {
            throw `No globule found for domain ${this.account.domain}`
        }

        generatePeerToken(globule, token => {

            if (globule.resourceService == null) {
                throw `No resource service found for domain ${this.account.domain}`
            }
            ...
            globule.resourceService
                ...
                .then((rsp: resource.UpdateSessionResponse) => {
                    ...
                    Backend.getGlobule(this.account.domain).eventHub.publish(`session_state_${this._id}_change_event`, this.toString(), false)
                    onSave();
                })
                .catch((err: any) => {
                    onError(err);
